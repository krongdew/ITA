...
if ($selectedYear != date('Y')) {
    ...
    $whereConditionPeople = "WHERE ((YEAR(Date) = $selectedYear-1 AND MONTH(Date) >= 10) OR (YEAR(Date) = $selectedYear  AND MONTH(Date) <= 9))";
    $whereConditionRespondent = "WHERE ((YEAR(SubmissionDate) = :selectedYearPrev AND MONTH(SubmissionDate) >= 10) OR (YEAR(SubmissionDate) = :selectedYear AND MONTH(SubmissionDate) <= 9))";
    $whereConditionResponse= "WHERE ((YEAR(date_data.assessment_date) = :selectedYearPrev AND MONTH(date_data.assessment_date) >= 10) OR (YEAR(date_data.assessment_date) = :selectedYear AND MONTH(date_data.assessment_date) <= 9))";
} else {
    // หากปีที่เลือกเป็นปีปัจจุบัน ให้ใช้เงื่อนไขของปีปัจจุบัน
    $whereConditionPeople = "WHERE ((MONTH(Date) >= 10 AND YEAR(Date) = $selectedYear-1) OR (MONTH(Date) <= 9 AND YEAR(Date) = $selectedYear ))";
    $whereConditionRespondent = "WHERE ((MONTH(SubmissionDate) >= 10 AND YEAR(SubmissionDate) = :selectedYearPrev) OR (MONTH(SubmissionDate) <= 9 AND YEAR(SubmissionDate) = :selectedYear))";
    $whereConditionResponse= "WHERE ((MONTH(date_data.assessment_date) >= 10 AND YEAR(date_data.assessment_date) = :selectedYearPrev) OR (MONTH(date_data.assessment_date) <= 9 AND YEAR(date_data.assessment_date) = :selectedYear))";
}
