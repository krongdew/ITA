...
// WHERE (YEAR(Date) = :selectedYear OR (MONTH(Date) BETWEEN MONTH(:startMonth) AND MONTH(:endMonth))) $userDepartmentCondition
...
try {
    ...
    ORDER BY 
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $data[] = array(
                "service_id" => $row['service_name'], // ใช้ข้อมูลที่ดึงมา
                "service_Access" => $row['service_Access'],
                "Jan" => $row['Jan'],
                "Feb" => $row['Feb'],
                "Mar" => $row['Mar'],
