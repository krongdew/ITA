...
$totalFiltered = $conn->query("SELECT COUNT(*) FROM sa_services WHERE 1 $userDepartmentCondition")->fetchColumn();
...
//     $whereCondition = "WHERE ((YEAR(Date) = $selectedYear AND MONTH(Date) >= 8) OR (YEAR(Date) = $selectedYear+1  AND MONTH(Date) <= 7))";
// } else {
//     // หากปีที่เลือกเป็นปีปัจจุบัน ให้ใช้เงื่อนไขของปีปัจจุบัน
//     $whereCondition = "WHERE ((MONTH(Date) >= 8 AND YEAR(Date) = $selectedYear) OR (MONTH(Date) <= 7 AND YEAR(Date) = $selectedYear+1 ))";
// }
